...
The easist way to start authoring a new post is to call the `create_post()` function from within your blog's directory. For example:
...
Beyond support for drafts, Radix has a number of additional features to accomodate a variety of post authoring and contribution workflows, including:

1) Authoring posts as R Markdown documents contained within the `_posts` directory (as described above) or authoring posts externally (e.g. within their own Git repositories).

2) Importing posts published elsewhere on the web (e.g. on RPubs, in a Git repository, or on another blog). Posts can be imported from their published form (i.e. you don't need to access or render the Rmd file used to author the post).

The article on [blog post workflow](blog_workflow.html) describes these options in more detail.
