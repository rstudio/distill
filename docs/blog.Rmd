...
You can add links that enable readers to subscribe to your blog by including the `collections:posts:subscribe` option within `_site.yml`. The `subscribe` option in turn points to an HTML file that describes and links to subscription options (e.g. [RSS feed](#rss-feeds) or email). For example:
