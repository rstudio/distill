...
You can also use [figure layout](figures.html) options with htmlwidgets. For example, here we set `distill.layout="l-page"` to specify that we want the widget to span the width of the page:
...
````clike
```{r, distill.layout="l-page", fig.height=6}`r ''`
library(r2d3)
...
You can use [Shiny](https://shiny.rstudio.com) components within your Distill articles to allow your readers to change the parameters underlying your analysis and see the results immediately. To use Shiny with Distill, first specify that you want to use the Shiny runtime with your document:
...
This turns your Distill article into an [interactive document](https://rmarkdown.rstudio.com/authoring_shiny.html), enabling you to add Shiny inputs and output directly to the document.
