...
The default layout if `distill.layout` is not specified is `l-body`, which will cause figures to span with width of the main article body:
...
For example, this plot specifies the `l-middle` layout using the `distill.layout` chunk option:
...
Note that when specifying an alternate `distill.layout` you should also specify and appropriate `fig.width` and `fig.height` for that layout.
