...
authors:
  - author: "JJ Allaire"
    authorURL: https://github.com/jjallaire
    affiliation: RStudio
    affiliationURL: https://www.rstudio.com
  - author: "Rich Iannone"
    authorURL: https://github.com/rich-iannone
    affiliation: RStudio
    affiliationURL: https://www.rstudio.com
  - author: "Yihui Xie"
    authorURL: https://github.com/yihui
    affiliation: RStudio
    affiliationURL: https://www.rstudio.com
...
- larger scale visualization layouts
    ...
    - crosstalk compatibility?
        - distillRow/distillRow

- JS error when there are no authors at all (need stub)
- Some default padding for html-widget (same as img)
- centered no longer works?
- create author section using pandoc
- test out aside tag
- take another shot at load latency (hiding the dom is not great)
...
The Distill web framework was originally created for use in <https://distill.pub> (a machine learning journal). Distill for [R Markdown](https://rmarkdown.rstudio.com/) enables you to easily author Distill articles using a fully reproducible workflow based on literate programming[@knuth1984].
...
For example, if we wanted to display a figure a bit outside the bounds of the article text, we could specify the `l-body-outset` layout via the `distill.layout` chunk option:
