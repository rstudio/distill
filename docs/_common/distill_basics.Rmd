...
Syntax highlighting for R code is done using the [downlit](https://downlit.r-lib.org) package, which provides automatic linking of R functions to their online documentation. The default highlighting theme matches the appearance of code within RStudio's default theme.
...
You can see a preview of available Pandoc highlighting themes here: <https://www.garrickadenbuie.com/blog/pandoc-syntax-highlighting-examples/> (note that dark themes won't work well with Distill articles!).

You can also create a custom highlighting theme file ([as described here](https://pandoc.org/MANUAL.html#syntax-highlighting)) and provide the path to that file as the `highlight` option.
