...
You can also deploy a Radix website to RStudio Connect from the command line using the `rsconnect::deploySite()` function. For example:
...
This example assumes you want to publish the site as it currently exists and publish to the default RStudio Connect account you've setup on your workstation. 

You can customize the deployment target using the `account` option and force a re-render of the site before publishing using the `render` option. For example:

```r
library(rsconnect)
deploySite(account = "dsmith", render = "local")
```
