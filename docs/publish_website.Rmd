...
| `creative_commons`      |  [Creative Commons](metadata.html#creative-commons) license terms for website content. |
...
Below we describe several convenient options for Radix website deployment. We'll mostly defer to the documentation provided by the various services, but will note any Radix website specific configuration required.
...
2. Set the `output_dir` within `_site.yml` to match the repository directory your site is served from. You can configure GitHub Pages to publish from the root of your master branch or the `docs` subdirectory. Depending on your configuration you should set the `output_dir` as follows:
