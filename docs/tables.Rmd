...
Often times tables will require more width for their display than the standard Distill article text width. Here we use `distill.layout="l-body-outset"` to cause the table to outset slightly from the main text column. All of the available [figure layouts](figures.html) can also be used with tables.
...
You can use Distill [figure layout](figures.html) classes with markdown tables by enclosing them in a `<div>` with the appropriate class, for example:
