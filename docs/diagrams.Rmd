...
The [r2d3](https://rstudio.github.io/r2d3) package enables you to include illustrations created using [D3.js](https://d3js.org/). For example, the following code renders [flare.json](https://github.com/jjallaire/distill/blob/master/docs/d3/circlepacking/flare.json) using the [circlepacking.js](https://github.com/jjallaire/distill/blob/master/docs/d3/circlepacking/circlepacking.js) script:

    ```{r, distill.layout="l-screen-inset", fig.height=10}`r ''`
    library(r2d3)
    data <- jsonlite::read_json("flare.json")
    r2d3(data, d3_version = 4, script = "circlepacking.js")
    ```

```{r, distill.layout="l-screen-inset", fig.height=10}
library(r2d3)
data <- jsonlite::read_json("d3/circlepacking/flare.json")
r2d3(data, d3_version = 4, script = "d3/circlepacking/circlepacking.js")
```

This is an illustration that has lots of detail so it benefits from being rendered at nearly full screen width (`distill.layout="l-screen-inset"`). We also specify `fig.height=10` to cause the illustration (which has a square aspect ratio) to take advantage of the additional horizontal space.
