...
Note that while you'll typically use the `distill::distill_article` format for pages within a website, you can also use any other HTML based R Markdown format so long as it's underlying template has support for [pandoc includes](https://pandoc.org/MANUAL.html#option--include-in-header). For example, you could use the [postcards](https://github.com/seankross/postcards) package to create a custom about page.

Note that alternate output formats requires the development version of both the distill and rmarkdown packages (and postcards should you try that out). You can install these as follows:
...
Note that you can disable support for rendering alternate formats by including `alt_formats: false` within your `_site.yml` file.
